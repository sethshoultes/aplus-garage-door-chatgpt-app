...
const InputSchema = z.object({
  location: z
    .string()
    .describe(
      "City name, neighborhood, or zip code (e.g., 'Henderson', 'Summerlin', '89123', 'Salt Lake City', 'Provo')"
    ),
});
...
function findServiceArea(location: string): {
  isCovered: boolean;
  ...
  // Check Nevada areas
  const nevadaAreas = serviceAreasData.regions.nevada.areas as ServiceArea[];
  ...
  for (const [regionKey, regionData] of Object.entries(utahRegions)) {
    const areas = (regionData as { areas: ServiceArea[] }).areas;
...
export const checkServiceAreaTool = {
  ...
  metadata: {
    ...
    description:
      "Verify if a location is within A Plus Garage Door's service area across Nevada and Utah. Use when user mentions a city, neighborhood, or zip code.",
