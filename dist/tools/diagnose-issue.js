...
const InputSchema = z.object({
    symptoms: z
        .array(z.string())
        .describe("List of symptoms (e.g., ['loud grinding noise', 'door won't close fully', 'remote not working'])"),
    door_age_years: z
        .number()
        .optional()
...
function analyzeDiagnosis(symptoms, doorAge) {
    let matchedIssues = new Set();
    let highestUrgency = "routine";
    let recommendedService = "maintenance";
    ...
    // Build issue details
    const issues = Array.from(matchedIssues)
        .slice(0, 3) // Limit to top 3 issues
        .map((issueKey) => {
        const issue = DIAGNOSTIC_TREE.issues[issueKey];
        return {
            name: issue.name,
...
export const diagnoseIssueTool = {
    ...
    handler: async (input) => {
        ...
        const diagnosis = analyzeDiagnosis(validated.symptoms, validated.door_age_years);
        return {
            structuredContent: {
...
//# sourceMappingURL=diagnose-issue.js.map